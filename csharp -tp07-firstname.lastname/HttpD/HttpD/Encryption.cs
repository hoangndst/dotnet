...
namespace HttpD
{
    public class Encryption
    {
        ...
        /// </summary>
        static void Xor(byte[] a, byte[] b, int offset) 
        {   
            for (int i = 0; i < a.Length; i++)
            {
                a[i] ^= b[i + offset];
        ...
        {
            byte[] expanded = new byte[11 * 16];
            Array.Copy(key, 0, expanded, 0, 16);

            for (int i = 16; i < expanded.Length; i += 16)
            {
                Array.Copy(expanded, i - 16, expanded, i, 16);
                KeySchedule(expanded, i / 16);
            }
